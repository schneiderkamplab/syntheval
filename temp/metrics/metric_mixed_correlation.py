...
class MixedCorrelation(MetricClass):
    ...
    def evaluate(self, mixed_corr=True) -> dict:
        ...
        if mixed_corr:
            ...
            plot_matrix_heatmap(corr_mat,'Mixed correlation matrix difference', 'corr_diff')
        else:
            r_corr = self.real_data[self.num_cols].corr()
            f_corr = self.synt_data[self.num_cols].corr()
            corr_mat = r_corr-f_corr
            plot_matrix_heatmap(corr_mat,'Correlation matrix difference (nums only)', 'corr_diff')
